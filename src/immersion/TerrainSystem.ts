...
export class TerrainSystem {
    ...
    public getHeight(x: number, z: number): number {
        // Map world coords to grid coords (assuming 0,0 is center of grid)
    ...
    public getRawHeight(ix: number, iz: number): number {
        if (ix < 0 || ix >= this.width || iz < 0 || iz >= this.depth) return 0;
