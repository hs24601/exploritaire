...
export class ImmersionEngine {
    ...
    private initLocations() {
        ...
        this.attribs = {
            ...
            rot: gl.getAttribLocation(p, 'aEntityRotation'),
    ...
    private initBuffers() {
        ...
        this.buffers.entities.rot = gl.createBuffer();
    ...
    public setCamera(cam: Partial<ImmersionCamera>) {
        if (cam.x !== undefined && cam.z !== undefined && cam.y === undefined) {
    ...
    public addEntity(entity: ImmersionEntity) {
        // Snap entity to terrain Y + small offset to prevent clipping
        const h = this.terrain.getHeight(entity.position[0], entity.position[2]);
        entity.position[1] = h + 0.1; // Float slightly above terrain
        
        // Ensure layers inherit parent position
        if (entity.layers) {
            entity.layers.forEach(layer => {
                layer.position = [...entity.position];
            });
        }
    ...
    public updateBuffers() {
        ...
        if (this.tiles.length > 0) {
            ...
            this.tiles.forEach((tile, i) => {
                ...
                const x = tile.position[0]; const z = tile.position[2];
                const h00 = this.terrain.getHeight(x, z); const h10 = this.terrain.getHeight(x + 1, z);
                const h01 = this.terrain.getHeight(x, z + 1); const h11 = this.terrain.getHeight(x + 1, z + 1);
                const verts = [0, h00, 0,  0, h01, 1,  1, h11, 1, 0, h00, 0,  1, h10, 0,  1, h11, 1];
                for (let j = 0; j < 6; j++) {
                    pos[b + j*3 + 0] = verts[j*3 + 0] + x; pos[b + j*3 + 1] = verts[j*3 + 1]; pos[b + j*3 + 2] = verts[j*3 + 2] + z;
                    ...
                    props[p+j*4+2] = -1.0; // Tile flag to prevent culling
                }
            });
        ...
        // Entities (Flatten & Sort)
        const flat: ImmersionEntity[] = [];
        const process = (ent: ImmersionEntity) => {
            flat.push(ent);
            if (ent.layers) ent.layers.forEach(process);
        };
        this.entities.forEach(process);

        flat.sort((a, b) => {
            const pA = a.properties.parallaxDepth || 0;
            const pB = b.properties.parallaxDepth || 0;
            // Subtract parallax bias to make foreground layers (positive p) appear "closer" than background layers
            const distA = Math.pow(a.position[0] - this.camera.x, 2) + Math.pow(a.position[1] - this.camera.y, 2) + Math.pow(a.position[2] - this.camera.z, 2) - pA * 0.5;
            const distB = Math.pow(b.position[0] - this.camera.x, 2) + Math.pow(b.position[1] - this.camera.y, 2) + Math.pow(b.position[2] - this.camera.z, 2) - pB * 0.5;
            return distB - distA;
        });
        ...
        if (flat.length > 0) {
            ...
            let totalVerts = 0;
            flat.forEach(ent => {
                const thickness = ent.properties.thickness || 0;
                const slices = Math.max(1, Math.ceil(thickness / sliceStep) + 1);
                totalVerts += slices * 6;
            });

            const pos = new Float32Array(totalVerts * 3);
            const uv = new Float32Array(totalVerts * 2);
            const trans = new Float32Array(totalVerts * 3);
            const rot = new Float32Array(totalVerts * 3);
            const color = new Float32Array(totalVerts * 4);
            const props = new Float32Array(totalVerts * 4);

            let vOffset = 0;
            flat.forEach((ent) => {
                const sx = ent.scale; const sy = ent.scale * (7/5);
                const tex = this.textureInfo.get(ent.textureId);
                const verts = [-sx/2, sy, 0,  -sx/2, 0, 0,  sx/2, 0, 0, -sx/2, sy, 0,   sx/2, sy, 0,  sx/2, 0, 0];
                
                const mode = ent.properties.renderMode || 0;
                const thickness = ent.properties.thickness || 0;
                const slices = Math.max(1, Math.ceil(thickness / sliceStep) + 1);

                let combined = mode; 
                if (ent.properties.hasShadowMitigation) combined += 10;
                ...
                if (ent.properties.isSupportStrut) combined += 40;

                for (let s = 0; s < slices; s++) {
                    const b = (vOffset + s * 6) * 3;
                    const u = (vOffset + s * 6) * 2;
                    const t = (vOffset + s * 6) * 3;
                    const r = (vOffset + s * 6) * 3;
                    const c = (vOffset + s * 6) * 4;
                    const p = (vOffset + s * 6) * 4;

                    for (let j = 0; j < 6; j++) {
                        pos[b+j*3+0]=verts[j*3+0]; pos[b+j*3+1]=verts[j*3+1]; pos[b+j*3+2]=verts[j*3+2];
                        if (tex) { 
                            uv[u+j*2+0]=tex.uvs[j*2+0]; uv[u+j*2+1]=tex.uvs[j*2+1]; 
                            props[p+j*4+1]=tex.atlasIndex; 
                        }
                        trans[t+j*3+0]=ent.position[0]; trans[t+j*3+1]=ent.position[1]; trans[t+j*3+2]=ent.position[2];
                        rot[r+j*3+0]=ent.rotation[0]; rot[r+j*3+1]=ent.rotation[1]; rot[r+j*3+2]=ent.rotation[2];
                        color[c+j*4+0]=ent.color[0]; color[c+j*4+1]=ent.color[1]; color[c+j*4+2]=ent.color[2]; color[c+j*4+3]=ent.opacity;
                        props[p+j*4+0]=s; // Slice Index
                        props[p+j*4+2]=ent.properties.parallaxDepth || 0; 
                        props[p+j*4+3]=combined;
                    }
                }
                vOffset += slices * 6;
            });
            ...
            gl.bindBuffer(gl.ARRAY_BUFFER, this.buffers.entities.rot); gl.bufferData(gl.ARRAY_BUFFER, rot, gl.DYNAMIC_DRAW);
            ...
            this.buffers.entities.count = totalVerts;
        } else {
            this.buffers.entities.count = 0;
        }
        
        // Setup Particles
        const rainCount = 1500; const splashCount = 500; const totalP = rainCount + splashCount;
        ...
        const sw = 0.05; const sverts = [-sw, sw, 0, -sw, 0, 0, sw, 0, 0, -sw, sw, 0, sw, sw, 0, sw, 0, 0];
        for (let i = 0; i < splashCount; i++) {
            ...
            for (let v = 0; v < 6; v++) {
                ...
                pData[idx+8]=rx + (Math.random()-0.5); pData[idx+9]=h + 0.01; pData[idx+10]=rz + (Math.random()-0.5);
                pData[idx+14]=1.0; pData[idx+15]=1; pData[idx+16]=1; pData[idx+17]=1; pData[idx+18]=0.8;
            }
        }
        gl.bindBuffer(gl.ARRAY_BUFFER, this.buffers.particles.interleaved); gl.bufferData(gl.ARRAY_BUFFER, pData, gl.STATIC_DRAW);
        this.buffers.particles.count = totalP * 6;
    }

    public render() {
        const gl = this.gl; const now = performance.now(); const delta = now - this.lastTime;
        this.lastTime = now; this.timeDelta += 0.001 * delta;
        ...
        this.renderParticles(pMatrix, mvMatrix, delta);
    }

    private renderWorld(pMatrix: THREE.Matrix4, mvMatrix: THREE.Matrix4) {
        const gl = this.gl; if (!this.lastLighting) return;
        gl.useProgram(this.program);
        gl.disable(gl.CULL_FACE); // Fix vanishing floor
        ...
        for (let i = 0; i < 8; i++) { if (this.textureAtlases[i]) { gl.activeTexture(gl.TEXTURE0+i); gl.bindTexture(gl.TEXTURE_2D, this.textureAtlases[i]); } }
        if (this.buffers.tiles.count > 0) {
            ...
            gl.disableVertexAttribArray(this.attribs.trans); gl.vertexAttrib3f(this.attribs.trans, 0, 0, 0);
            gl.disableVertexAttribArray(this.attribs.rot); gl.vertexAttrib3f(this.attribs.rot, 0, 0, 0);
        ...
        if (this.buffers.entities.count > 0) {
            ...
            this.bindAttrib(this.attribs.rot, this.buffers.entities.rot, 3);
    ...
    private renderParticles(pMatrix: THREE.Matrix4, mvMatrix: THREE.Matrix4, delta: number) {
        const gl = this.gl; if (!this.lastLighting) return;
        gl.useProgram(this.particleProgram); gl.depthMask(false);
        ...
        const stride = 19 * 4; gl.bindBuffer(gl.ARRAY_BUFFER, this.buffers.particles.interleaved);
        this.setupPAttrib(this.pAttribs.pos, 3, stride, 0); this.setupPAttrib(this.pAttribs.initPos, 3, stride, 12);
        this.setupPAttrib(this.pAttribs.vel, 1, stride, 24); this.setupPAttrib(this.pAttribs.type, 1, stride, 28);
        this.setupPAttrib(this.pAttribs.dest, 3, stride, 32); this.setupPAttrib(this.pAttribs.trans, 3, stride, 44);
        this.setupPAttrib(this.pAttribs.life, 1, stride, 56); this.setupPAttrib(this.pAttribs.color, 4, stride, 60);
        gl.drawArrays(gl.TRIANGLES, 0, this.buffers.particles.count); gl.depthMask(true);
    }

    private setupPAttrib(loc: number, size: number, stride: number, offset: number) { if (loc === -1) return; this.gl.enableVertexAttribArray(loc); this.gl.vertexAttribPointer(loc, size, this.gl.FLOAT, false, stride, offset); }
    private bindAttrib(loc: number, buffer: WebGLBuffer, size: number) { if (loc === -1) return; this.gl.enableVertexAttribArray(loc); this.gl.bindBuffer(this.gl.ARRAY_BUFFER, buffer); this.gl.vertexAttribPointer(loc, size, this.gl.FLOAT, false, 0, 0); }
