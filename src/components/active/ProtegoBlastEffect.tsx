...
export type ProtegoBlastConfig = {
...
export const DEFAULT_PROTEGO_BLAST_CONFIG: ProtegoBlastConfig = {
...
type Props = {
  className?: string;
  config?: ProtegoBlastConfig;
};

export const ProtegoBlastEffect = memo(function ProtegoBlastEffect({ 
  className,
  config: userConfig = DEFAULT_PROTEGO_BLAST_CONFIG 
}: Props) {
  ...
  useEffect(() => {
    ...
    const config = {
      ...
      DENSITY_DISSIPATION: userConfig.densityDissipation,
      VELOCITY_DISSIPATION: userConfig.velocityDissipation,
      PRESSURE_DISSIPATION: userConfig.pressureDissipation,
      PRESSURE_ITERATIONS: userConfig.pressureIterations,
      CURL: userConfig.curl,
      SPLAT_RADIUS: userConfig.splatRadius,
    };
    ...
    const getSupportedFormat = (gl: WebGL2RenderingContext, internalFormat: number, format: number, type: number): any => {
  ...
  return (
    <div className={`w-full h-full bg-black flex items-center justify-center p-10 ${className ?? ''}`}>
      <div className="relative w-full h-full border border-game-teal/20 rounded-xl overflow-hidden bg-black">
        <canvas ref={canvasRef} className="w-full h-full block" />
        <div className="absolute inset-0 pointer-events-none flex items-center justify-center">
          <div className="text-game-teal font-mono text-[10px] uppercase tracking-widest opacity-30">Active Effect: protego_blast</div>
        </div>
      </div>
    </div>
  );
